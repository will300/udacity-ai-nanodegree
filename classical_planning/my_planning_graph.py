...
class ActionLayer(BaseActionLayer):
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        for itemA in self.parents[actionA]:
            for itemB in self.parents[actionB]:
                if self.parent_layer.is_mutex(itemA, itemB):
                    return True
        ...
        for actionA in self.parents[literalA]:
            for actionB in self.parents[literalB]:
                if not self.parent_layer.is_mutex(actionA, actionB):
                    all_actions_mutex = False
                
        if all_actions_mutex:
            return True
